...
from 
  (
    SELECT 
      parse_json($1) data 
    FROM 
      @veots_stage/prod-veots/static_coll/admins
  );
...
data:updatedAt::TIMESTAMP_NTZ
FROM
(SELECT parse_json($1) data FROM @veots_stage/prod-veots/static_coll/masterclients);
...
from
(SELECT parse_json($1) data FROM @veots_stage/prod-veots/static_coll/batches);
...
from
(SELECT parse_json($1) data FROM @veots_stage/prod-veots/static_coll/products);
...
F.VALUE:customerId:oid::varchar fakereported_customer,
...
from
(SELECT parse_json($1) data FROM @veots_stage/prod-veots/static_coll/subclients);
...
from
(SELECT parse_json($1) data FROM @veots_stage/prod-veots/static_coll/onetimeclients);
...
from
(SELECT parse_json($1) data FROM @veots_stage/prod-veots/static_coll/customers);
...
INSERT  INTO SKUS
 SELECT
	...
	DATA:updatedAt ::TIMESTAMP_NTZ
from
(SELECT parse_json($1) data FROM @veots_stage/prod-veots/static_coll/skus);
...
create or replace table typeonecouponsets(
_id object,
masterClient object,
couponLogoLink VARCHAR,
couponHeader VARCHAR,
couponText VARCHAR,
distributionType VARCHAR,
distributionRatio VARCHAR,
expiryDate TIMESTAMP_NTZ(9),
offerDetails VARCHAR,
...
data:updatedAt::TIMESTAMP_NTZ
from (
select parse_json($1) data from @veots_stage/staging-veots/static_coll/typeonecouponsets/
);


create or replace view typeonecouponsets_v as
SELECT _ID:oid ::VARCHAR oid ,
	masterClient:oid::VARCHAR masterclient_id,
    couponLogoLink,
couponHeader ,
couponText ,
distributionType ,
...
createdAt ,
updatedAt 
FROM (SELECT *, ROW_NUMBER() OVER(PARTITION BY _ID:oid ORDER BY UPDATEDAT DESC) ISACTIVE FROM typeonecouponsets)
where ISACTIVE = 1;
...
create or replace table typeonecouponcodes
(
    _id object,
    assignedTo object,
    couponCode VARCHAR,
    deliveredAt TIMESTAMP_NTZ(9),
    isAssigned BOOLEAN,
    isScratched BOOLEAN,
    productId OBJECT,
    setId OBJECT,
...
insert into typeonecouponcodes
select 
    data:_id::VARIANT,
    ...
    data:updatedAt::TIMESTAMP_NTZ   
    from (
select parse_json($1) data from @veots_stage/staging-veots/static_coll/typeonecouponcodes/);



create or replace view typeonecouponcodes_v as
select 
    _id:oid::VARCHAR OID,
    assignedTo:oid::VARCHAR assignedTo,
    couponCode,
    deliveredAt,
    isAssigned,
    isScratched,
    productId:oid::VARCHAR PRODUCTID,
    setId:oid::VARCHAR SETID,
    createdAt,
    updatedAt
    from (SELECT *, ROW_NUMBER() OVER(PARTITION BY _ID:oid ORDER BY UPDATEDAT DESC) ISACTIVE FROM typeonecouponcodes)
    where ISACTIVE = 1;



CREATE or replace view coupons_v as 
select 
