...
from 
  (
    ...
    FROM 
      @veots_stage/staging-veots/dors 
  );
...
from 
  (
    SELECT 
      parse_json($1) data,SPLIT(METADATA$FILENAME,'/')[2]::VARCHAR  CLIENT_ID
    FROM 
      @veots_stage/staging-veots/tors 
  );
...
from 
  (
    SELECT 
      parse_json($1) data,SPLIT(METADATA$FILENAME,'/')[2]::VARCHAR  CLIENT_ID
    FROM 
      @veots_stage/staging-veots/vectortable4_1/ 
  );  
...
from 
  (
    SELECT 
      parse_json($1) data,SPLIT(METADATA$FILENAME,'/')[2]::VARCHAR  CLIENT_ID
    FROM 
      @veots_stage/staging-veots/vectortable4_2/ 
  );
...
from 
  (
    SELECT 
      parse_json($1) data,SPLIT(METADATA$FILENAME,'/')[2]::VARCHAR  CLIENT_ID
    FROM 
      @veots_stage/staging-veots/vectortable6/
  );
...
from 
  (
    SELECT 
      parse_json($1) data,SPLIT(METADATA$FILENAME,'/')[2]::VARCHAR  CLIENT_ID
    FROM 
      @veots_stage/staging-veots/vectortable7/
  );
...
from 
  (
    SELECT 
      parse_json($1) data,SPLIT(METADATA$FILENAME,'/')[2]::VARCHAR  CLIENT_ID
    FROM 
      @veots_stage/staging-veots/vectortable8/
  );
...
-- with cte as (
-- SELECT V.VECTORTABLE8_OID, 
--     d.*
--     FROM
-- (SELECT _ID:oid::VARCHAR VECTORTABLE8_OID , scannerDetails FROM VECTORTABLE8) V,
-- LATERAL FLATTEN (scannerDetails) C ,
-- LATERAL FLATTEN (C.VALUE) d)
-- select distinct key from cte
----------------VECTORTABLE9-------------------

-- with cte as (
--  SELECT 
--       parse_json(a.$1) data,SPLIT(METADATA$FILENAME,'/')[2]::VARCHAR  CLIENT_ID,
--       b.*
--     FROM 
--       @veots_stage/staging-veots/vectortable9/ a,
...
from 
  (
    SELECT 
      parse_json($1) data,SPLIT(METADATA$FILENAME,'/')[2]::VARCHAR  CLIENT_ID
    FROM 
      @veots_stage/staging-veots/vectortable9/
  );
